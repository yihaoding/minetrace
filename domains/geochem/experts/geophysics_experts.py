...
class _GeophysicsExpert:
    ...
    def score_node(self, sample) -> Optional[NodeScore]:
        ...
        if self._invert_score:
            raw = -raw
            # keep z signs consistent with the score they explain
            z_scores = {f: -z for f, z in z_scores.items()}
        sc = float(1.0 / (1.0 + np.exp(-raw)))
        # normalized over features actually present, so Σ feature_w · z == raw
        feature_w = {f: self._shape_w.get(f, 0.0) / total_w for f in z_scores}
        return NodeScore(
