...
class TargetEnrichmentExpert:
    ...
    def score_node(self, sample) -> Optional[NodeScore]:
        ...
        for sn, st in self._states.items():
            ...
            sub_weights.append(st.source_weight)
            for k, v in z.items():
                all_z[f"{sn}:{k}"] = v
        if not sub_scores:
            return None
        w_arr = np.array(sub_weights)
        score = float(np.average(sub_scores, weights=w_arr / w_arr.sum()))
        return NodeScore(
            name=self.name, score=score,
            weights={sn: w for sn, w in zip(self._states, sub_weights)},
            feature_z=all_z,
        )
...
class ElementCorrelationExpert:
    ...
    def score_node(self, sample) -> Optional[NodeScore]:
        ...
        for sn, state in self._pair_states.items():
            ...
            for (e1, e2), w in state["w"].items():
                ...
                if np.isfinite(v1) and np.isfinite(v2):
                    all_z[f"{sn}:{e1}_{e2}"] = float(state["d"][(e1, e2)] * v1 * v2)
        if not sub_scores:
            return None
        w_arr = np.array(sub_weights)
        score = float(np.average(sub_scores, weights=w_arr / w_arr.sum()))
        return NodeScore(
            name=self.name, score=score,
            weights={sn: w for sn, w in self._source_weights.items()},
            feature_z=all_z,
        )
