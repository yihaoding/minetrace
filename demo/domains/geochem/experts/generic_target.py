...
def _weighted_sigmoid(
    ...
    if invert:
        raw = -raw
    return float(1.0 / (1.0 + np.exp(-raw))), {f: shape_w.get(f, 0.0) for f in features}, z_scores
...
class TargetEnrichmentExpert:
    ...
    def score_node(self, sample) -> Optional[NodeScore]:
        ...
        if result is None:
            return None
        sc, weights, z_scores = result
        return NodeScore(name=self.name, score=sc, weights=weights, feature_z=z_scores)
