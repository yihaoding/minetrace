...
def _expert_source(name: str) -> str:
    """Extract data source from expert name, e.g. 'cu_rockchips_enrichment' → 'rockchips'."""
    for token in (
...
def describe(
    ...
    for child_name, child_ns in node_score.children.items():
        ...
        all_z = child_ns.all_feature_z()
        feat_signals = [
            FeatureSignal(
                feature=feat,
                z_score=z,
                weight=child_ns.weights.get(feat, 0.0),
                expert_name=expert_name,
                source=_expert_source(expert_name),
            )
            for feat, (z, expert_name) in all_z.items()
            if abs(z) > 0.1
        ]
    ...
    all_z_global = node_score.all_feature_z()
    # build full FeatureSignal list; get weight from the leaf's own NodeScore
    all_signals: list[FeatureSignal] = []
    def _collect_weights(ns, depth=0):
        for feat, z in ns.feature_z.items():
            all_signals.append(FeatureSignal(
                feature=feat,
                z_score=z,
                weight=ns.weights.get(feat, 0.0),
                expert_name=ns.name,
                source=_expert_source(ns.name),
            ))
        for child in ns.children.values():
            _collect_weights(child, depth + 1)

    _collect_weights(node_score)
    all_signals.sort(key=lambda s: abs(s.contribution), reverse=True)

    # deduplicate by feature name (keep highest |contribution|)
    seen_feats: set[str] = set()
    top_signals: list[FeatureSignal] = []
    for sig in all_signals:
        if sig.feature not in seen_feats:
            seen_feats.add(sig.feature)
